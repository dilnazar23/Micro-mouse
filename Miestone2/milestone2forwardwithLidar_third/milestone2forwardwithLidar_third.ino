...
// Set up pid for lidar
mtrn3100::PIDController pid_lidar(0.5, 0, 0, 0);
...
size_t poseIndex = 0;
const MazeCellPose poseSequence[poseSequenceLength] = {{0, 0, 0}, {250, 0, 0}};  
...
void loop() {
    ...
    // while (1){};
    // Debugging is lidar reading updated in wile loop?
    // while(1){
    //   Serial.println(sensor0.readRangeContinuousMillimeters()+sensor1.readRangeContinuousMillimeters());
    //   delay(1000);
    //   Serial.println(sensor0.readRangeContinuousMillimeters()+sensor1.readRangeContinuousMillimeters());
    // };
      const float l_curr_encoder = l_encoder.position;
      const float r_curr_encoder = r_encoder.position;
      
      // fixing heading using pid only
      float prev_lid = 10000;
      float curr_lid = sensor0.readRangeContinuousMillimeters()+sensor1.readRangeContinuousMillimeters();
      while(prev_lid > curr_lid){ 
        //prev_lid = curr_lid;

        Serial.print("current Lidar ");
        Serial.println(curr_lid);
        float r_lidar = sensor0.readRangeContinuousMillimeters();
        float l_lidar = sensor1.readRangeContinuousMillimeters();
        prev_lid = r_lidar + l_lidar;
                Serial.print("previous Lidar ");
        Serial.println(prev_lid);
        float speed_fix_h = pid_lidar.lidar_compute(prev_lid);
        l_motor.setPWM(speed_fix_h); 
        r_motor.setPWM(-speed_fix_h);
        Serial.print("Speed: ");
        Serial.println(speed_fix_h);
        delay(100);
        curr_lid = sensor0.readRangeContinuousMillimeters()+sensor1.readRangeContinuousMillimeters();
        Serial.println(curr_lid);
      };
      l_motor.setPWM(0);
      r_motor.setPWM(0);
      delay(3000);
            // change my current position to before fixing heading
      l_encoder.position = l_curr_encoder;
      r_encoder.position = r_curr_encoder;
      
    //while(1){}
    while((fabs(l_expect - (l_encoder.position-thetaL)) >=0.05) || (fabs(r_expect - (r_encoder.position-thetaR)) >=0.05)){
      
      // without lidar fixing
