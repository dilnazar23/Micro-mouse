...
constexpr size_t poseSequenceLength = 11;
size_t poseIndex = 0;
const MazeCellPose poseSequence[poseSequenceLength] = {{250, 250, 0}, {500, 250, 0}, {500, 250,-M_PI/2},{750,250,-M_PI/2}, {750, 250, -M_PI},{1000, 250, -M_PI},{1000, 250, -M_PI/2}, {1000, 250, 0},{1250, 250, 0}, {1250, 250, M_PI/2},{1250, 250, M_PI}};                                                      
...
void loop() {
    ...
    while((fabs(l_expect - (l_encoder.position-thetaL)) >=0.05) || (fabs(r_expect - (r_encoder.position-thetaR)) >=0.05)){
      ...
      float l_lida_err = 0;
      float r_lida_err = 0;
      if (!isTranslationalMovement){}
      else if (l_lidar<255 && r_lidar<255){
