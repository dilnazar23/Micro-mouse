...
public:
    ...
    float* compute(float error[2]) {
        ...
        integral[0] = integral[0] + error[0]*dt;    // COMPLETE THIS LINE.
        derivative[0] = (error[0]- prev_error[0])/dt;  // COMPLETE THIS LINE.
        output[0] = kp*error[0] + ki*integral[0] + kd* derivative[0];      // COMPLETE THIS LINE.

        prev_integral[0] = integral[0];
        prev_error[0] = error[0];

        integral[1] = integral[1] + error[1]*dt;    // COMPLETE THIS LINE.
