...
void setup() {
  Serial.begin(9600);
  //Serial.begin(9600);
  int source = 1;        // starting position
  int destination = 10;  // destination position
  // int ascii_size=sizeof(maze);
  // while(1){Serial.println(ascii_size);}
  auto const actual = mtrn3100::ascii2graph(maze);
  mtrn3100::LinkedList<mtrn3100::LinkedList<int>> shortestPaths = bfs_single(actual, source, destination);
