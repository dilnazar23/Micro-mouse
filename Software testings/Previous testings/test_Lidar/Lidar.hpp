...
public:
    //template <T>
    Lidar(uint8_t enablePin, char * address) : enablePin(enablePin), address(address){
        ...
        configureDefault();
        setAddress(address);
        Serial.println(readReg(0x212),HEX); // read I2C address
        writeReg(VL6180X::SYSRANGE__MAX_CONVERGENCE_TIME, 30);
        writeReg16Bit(VL6180X::SYSALS__INTEGRATION_PERIOD, 50);
        setTimeout(500);
        stopContinuous();
        setScaling(RANGE); // configure range or precision 1, 2 oder 3 mm
        delay(300);
...
//          Serial.println(lidar_sensor.readReg(0x212),HEX); // read I2C address
//          lidar_sensor.writeReg(VL6180X::SYSRANGE__MAX_CONVERGENCE_TIME, 30);
//          lidar_sensor.writeReg16Bit(VL6180X::SYSALS__INTEGRATION_PERIOD, 50);
//          lidar_sensor.setTimeout(500);
//          lidar_sensor.stopContinuous();
//          lidar_sensor.setScaling(RANGE); // configure range or precision 1, 2 oder 3 mm
//          delay(300);
//          lidar_sensor.startInterleavedContinuous(100);
//          delay(100);
//  
//      }
//
//      float lidar_read() {
//        return lidar_sensor.readRangeContinuousMillimeters();
//      }
//
//      void lidar_init() {
//        lidar_sensor.init();
//      }
//
//      void lidar_configureDefault () {
//        lidar_sensor.configureDefault();
//      }
//
//      void lidar_setTimeout (uint16_t time) {
//        lidar_sensor.setTimeout(uint16_t time);
//      }
//
//
//      
