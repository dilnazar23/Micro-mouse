...
float readLIDAR_diff(){
  float dis0=sensor0.readRangeContinuousMillimeters();
  float dis1=sensor1.readRangeContinuousMillimeters();
  float diss = fabs(dis0-dis1);
  // Serial.println(diss);
  // Serial.println();
...
void fixHead_lidar(bool leftwall){
  float prevDistance = readLIDAR_diff();
  ...
  while (true) {
    ...
    const float currentDistance = readLIDAR_diff();
    Serial.println(currentDistance);
    if (currentDistance -prevDistance>=5) {
...
void fixHead_imu(){
  ...
  if (abs(h_imu-h1)<0.1) {break;}// change this value here was 0.1 
  }
}
