...
class IMU {
public:
    IMU(bool adrValue ) : adrValue(adrValue) {}
    ...
    void begin() {
        ...
        Serial3.println(F("Device connected!"));
        bool success = true;
        ...
        // Check success
        if (success) {
            Serial3.println(F("DMP enabled!"));
        } else {
            Serial3.println(F("Enable DMP failed!"));
            Serial3.println(
    ...
    void read() {
        ...
        auto mg2mps2 = [](float const val) { return val / 9.81; };
        ...
        // if (!isnan(y)) {
        //     // COMPLETE THIS BLOCK.
        //     px = px + (vx * dt + (1/2)*ax*dt*dt)*cos(y);
        //     py = py + (vy * dt + (1/2)*ay*dt*dt)*sin(y);
        // }
    }
